...
//
//          Para compilar:
//              g++ -o Recoil_force_dipole_approximation Recoil_force_dipole_approximation.cpp IN31.cpp -lfftw3 -lm -lcomplex_bessel -fopenmp
//          Para ejecutar:
//              ./Recoil_force_dipole_approximation
//
//---------------------------------------------------------------------------------------------------------
...
int main() {
    ...
    for (int s = 0; s < N; s++) {
        ...
        OmegaE11_values_in_time_domain[s] *= delta_omega/(2*Pi); // Factor de escala dado por la partición en el dominio espectral y convención de transformada
        if (abs(OmegaE11_values_in_time_domain[s].real()) < threshold) OmegaE11_values_in_time_domain[s].real(0.0);
        OmegaM11_values_in_time_domain[s] *= delta_omega/(2*Pi); // Factor de escala dado por la partición en el dominio espectral y convención de transformada
    ...
    #pragma omp parallel for
    for (int j = 0; j<N ; j++){
        Fuerza_de_retroceso_X[j]=factor_rel*(OmegaM11_values_in_time_domain[j].real())*(OmegaE10_values_in_time_domain[j].real())/gammaLorentz(beta);   //Componente x (transversal)
        Fuerza_de_retroceso_Z[j]=-factor_rel*(OmegaM11_values_in_time_domain[j].real())*(OmegaE11_values_in_time_domain[j].real());                     //Componente z (longitudinal)
    }
    ...
    ofstream file("recoil_force_dipolar_aprox_results.txt");
    for (int i = s_min; i < s_max; i++) {
        file << time_domain_values[i] << " " << Fuerza_de_retroceso_X[i] << " " << Fuerza_de_retroceso_Z[i] << "\n";
    }
    file.close();
    ...
    cout << "Transformada inversa de Fourier completada y resultados guardados en recoil_force_dipolar_aprox_results.txt" << endl;
